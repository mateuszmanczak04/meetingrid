defmodule Core.Meetings.Attendee.ConfigTest do
  use ExUnit.Case, async: true

  alias Core.Meetings.Attendee.Config

  describe "cast/1" do
    test "casts struct successfully" do
      week_config = %Config.Week{mode: :week, available_days: [0, 2]}
      assert {:ok, ^week_config} = Config.cast(week_config)
    end

    test "casts map successfully" do
      data = %{"mode" => "week"}
    ...
    test "returns error for invalid changeset" do
      data = %{"mode" => "week", "available_days" => "invalid"}
      assert :error = Config.cast(data)
    end
  ...
  describe "load/1" do
    test "loads config from database map" do
      data = %{"mode" => "week", "available_days" => [1, 3, 4]}
      assert {:ok, %Config.Week{mode: :week, available_days: [1, 3, 4]}} = Config.load(data)
    end
  ...
  describe "dump/1" do
    test "dumps struct to map" do
      week_config = %Config.Week{mode: :week, available_days: [3, 6]}
